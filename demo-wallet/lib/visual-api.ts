import { demoWalletConfig, buildAuthHeaders } from "@/lib/config";
...
class VisualApiError extends Error {
  statusCode: number;
...
const requestJson = async <T>(path: string, options: RequestOptions = {}): Promise<T> => {
  const url = `${demoWalletConfig.visualApiBase}${path}`;
...
export const initVisualAuth = async (input: {
  ...
  const resolvedCallbackUrl =
    input.callbackUrl ?? `${demoWalletConfig.publicOrigin}/api/demo-wallet/login/finalize`;
  ...
  const response = await requestJson<InitAuthResponse>("/visual-password/v1/init-auth", {
    ...
    body: {
      partnerId: demoWalletConfig.partnerId,
  ...
  const verifyUrl = new URL(response.verifyPath, demoWalletConfig.visualVerifyOrigin).toString();
...
export const initVisualEnroll = async (input: { partnerUserId: string; state: string }) => {
  const callbackUrl = `${demoWalletConfig.publicOrigin}/api/demo-wallet/enroll/callback`;
  ...
  const enrollUrl = new URL(response.enrollPath, demoWalletConfig.visualVerifyOrigin).toString();
