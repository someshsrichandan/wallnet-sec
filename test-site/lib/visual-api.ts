...
const requestJson = async <T>(
  ...
  const url = `${siteConfig.visualApiBase}${path}`;
...
export const initVisualAuth = async (input: {
  ...
  const resolvedCallbackUrl =
    input.callbackUrl ?? `${siteConfig.publicOrigin}/api/wallet/login/finalize`;

  const response = await requestJson<InitAuthResponse>(
...
export const initVisualEnroll = async (input: {
  partnerUserId: string;
  state: string;
}) => {
  const callbackUrl = `${siteConfig.publicOrigin}/api/wallet/enroll/callback`;
