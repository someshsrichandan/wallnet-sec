...
const requestJson = async <T>(path: string, options: RequestOptions = {}): Promise<T> => {
  const url = `${demoBankConfig.visualApiBase}${path}`;
...
export const initVisualAuth = async (input: {
  ...
  const resolvedCallbackUrl =
    input.callbackUrl ?? `${demoBankConfig.publicOrigin}/api/demo-bank/login/finalize`;
  ...
  // Always redirect to the SaaS verify page — partners don't host their own UI.
  const verifyUrl = new URL(response.verifyPath, demoBankConfig.visualVerifyOrigin).toString();
...
export const initVisualEnroll = async (input: { partnerUserId: string; state: string }) => {
  const callbackUrl = `${demoBankConfig.publicOrigin}/api/demo-bank/enroll/callback`;
  ...
  const enrollUrl = new URL(response.enrollPath, demoBankConfig.visualVerifyOrigin).toString();
